...
parser.add_argument('--npz_checkpoint', type=str, default=False,                               help='use this option to load a checkpoint npz for metric computation, skip map computation if used')
        ...
        for X, y in tqdm(val_loader, total=len(val_loader), desc=f'Generating saliency maps using {args.method}'):
            ...
            for i in range(X.shape[0]): 
                ...
                if args.method in ['scorecam', 'gradcam', 'gradcam++']:
                    ...
                    print('[DEBUG] RESULT ', saliency_map)
